...
# --- Gráfica Escenario A ---
fig_A, axA = plt.subplots(figsize=(8, 6))
axA.plot(t_eval, resultados_A[:, 0], label='Susceptibles (S)', color='blue')
axA.plot(t_eval, resultados_A[:, 1], label='Expuestos (E)', color='orange')
axA.plot(t_eval, resultados_A[:, 2], label='Infectados (I)', color='red')
axA.plot(t_eval, resultados_A[:, 3], label='Recuperados (R)', color='green')
axA.set_title('Escenario A: Propagación Libre ($\\beta = 0.6$)')
axA.set_xlabel('Días')
axA.set_ylabel('Población')
axA.grid(True)
axA.legend()
fig_A.tight_layout()
fig_A.savefig('escenario_A.pdf', format='pdf', bbox_inches='tight')

# --- Gráfica Escenario B ---
fig_B, axB = plt.subplots(figsize=(8, 6))
axB.plot(t_eval, resultados_B[:, 0], label='Susceptibles (S)', color='blue')
axB.plot(t_eval, resultados_B[:, 1], label='Expuestos (E)', color='orange')
axB.plot(t_eval, resultados_B[:, 2], label='Infectados (I)', color='red')
axB.plot(t_eval, resultados_B[:, 3], label='Recuperados (R)', color='green')
axB.axvline(x=30, color='gray', linestyle='--', label='Inicio Intervención (t=30)')
axB.set_title('Escenario B: Con Intervención en t=30 ($\\beta = 0.25$)')
axB.set_xlabel('Días')
axB.set_ylabel('Población')
axB.grid(True)
axB.legend()
fig_B.tight_layout()
fig_B.savefig('escenario_B.pdf', format='pdf', bbox_inches='tight')
